<?php
    ...
    $body_class = "site-register-number";
...
?>
    ...
    <section class="search-result" aria-label="전화번호 검색 결과">
        ...
        <?php else: ?>
            <p class="mb10">입력하신 번호에 대한 <strong class="line">등록된 정보가 아직 없습니다.</strong></p>
        <?php endif; ?>
    </section>
    ...
    <section class="search-input" aria-label="번호 정보 제보 폼">
        ...
        <form action="/pages/register_save.php" method="post" class="register-form">
            <input type="hidden" name="number" value="<?= htmlspecialchars($q, ENT_QUOTES, 'UTF-8') ?>">

            <div class="form-group">
                <label for="store_name">유형</label>
                <select id="store_name" name="store_name" required>
                    <option value="">선택하세요</option>
                    <option value="스팸·광고">스팸·광고</option>
                    <option value="보이스피싱·사기">보이스피싱·사기</option>
                    <option value="택배·배달">택배·배달</option>
                    <option value="업체·상점">업체·상점</option>
